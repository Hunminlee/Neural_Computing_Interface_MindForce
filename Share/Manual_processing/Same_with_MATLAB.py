...
class EMGFeatureExtractor:
    def __init__(self, feat_mean, feat_std, filter_b, filter_a, Norm_bool, num_feature_set):
        ...
        self.buffer = None  # to be set externally - raw data : mat['Data_ADC']
        self.normalization = Norm_bool
        self.num_feature_set = num_feature_set

    #Default : win_size=600, win_step=120, feat_exclude=60
    def extract_features(self, win_size, win_step, feat_exclude):
    ...
    def extract_features_with_labels(self, win_size, win_step, feat_exclude, filtering, restored_label=None):
        if filtering:
            buf = lfilter(self.filter_b, self.filter_a, self.buffer, axis=1)
        else:
            buf = self.buffer
        ...
        # label 배열 초기화 (optional)
        labels = np.zeros(n_steps, dtype=np.int32) if restored_label is not None else None
        ...
        for i in range(n_steps):
            ...
            # 레이블 동기화
            if restored_label is not None:
                window_label = restored_label[start:end]
                labels[i] = np.bincount(window_label).argmax()  # majority voting
        ...
        if self.normalization:
            if self.num_feature_set == 14:
                ...
                features[:, :14, :] = (features[:, :14, :] - self.feat_mean[:, :, np.newaxis]) / self.feat_std[:, :,
                                                                                                 np.newaxis]
                new_feats = features[:, 14:, :]
                new_mean = np.mean(new_feats, axis=2, keepdims=True)
                new_std = np.std(new_feats, axis=2, keepdims=True) + 1e-6
                features[:, 14:, :] = (new_feats - new_mean) / new_std

        if features.shape[2] > feat_exclude:
            features = features[:, :, feat_exclude - 1:]
            if labels is not None:
                labels = labels[feat_exclude - 1:]
